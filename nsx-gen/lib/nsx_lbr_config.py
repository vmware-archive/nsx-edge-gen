...
LBR_CONFIG_FILE = "lbr_config_template.yml"
...
class Config(dict):
	...
	def read_config(self, input_config_file=LBR_CONFIG_FILE):
		#print('Input config file:{}'.format(input_config_file))
		input_config_file = TEMPLATE_PATH + '/' + LBR_CONFIG_FILE

		try:
			with open(input_config_file) as config_file:
				self.update(read_yaml(config_file))
		except IOError as e:			
			print >> sys.stderr, 'Not a lbr config file.'
			sys.exit(1)

	def read(self, config_file=LBR_CONFIG_FILE):
...
def load_lbr_cfg():
	lbr_cfg = Config().read()
	#print('lbr_config : {}'.format(lbr_cfg))
	return lbr_cfg
